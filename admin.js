...
function saveCustomProducts(products) {
    try {
        ...
        alert("CRITICAL ERROR: Could not save products. Your browser storage might be full (Base64 images are large). Try using smaller images or removing some products.");
    }
}
...
function updateDashboard() {
    ...
    list.forEach(p => {
        ...
        tr.innerHTML = `
            ...
            <td>₹${p.price}</td>
            <td>
                <button class="edit-btn" onclick="editProduct(${p.id}, '${p.type}')">
                    Edit
                </button>
...
function setupForm() {
    ...
    if (editForm) {
        editForm.addEventListener('submit', (e) => {
            ...
            if (type === 'static') {
                // To "edit" a static product, we hide the static one and add a new custom one
                const hidden = getHiddenIds();
                if (!hidden.includes(id)) {
                    hidden.push(id);
                    saveHiddenIds(hidden);
                }
                // Use original ID or new ID? Amazon-style would use original but here IDs are static.
                // Let's use a new ID for the custom clone to avoid collision with hidden static.
